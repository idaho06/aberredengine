...
impl LuaRuntime {
    ...
    pub(super) fn register_builder_meta(&self) -> LuaResult<()> {
        ...
        let builder_methods: &[BuilderMethodDef] = &[
            ...
            ("with_sprite", "Set sprite", &[
                ("tex_key", "string"), ("width", "number"), ("height", "number"),
            ...
            ("with_accel", "Add a named acceleration force", &[
                ("name", "string"), ("x", "number"), ("y", "number"), ("enabled", "boolean"),
            ]),
            ("with_frozen", "Mark entity as frozen (physics skipped)", &[]),
            ("with_collider", "Set box collider", &[
                ("width", "number"), ("height", "number"), ("origin_x", "number"), ("origin_y", "number"),
            ]),
            ...
            ("with_text", "Set DynamicText component", &[
                ("content", "string"), ("font", "string"), ("font_size", "number"),
                ("r", "integer"), ("g", "integer"), ("b", "integer"), ("a", "integer"),
            ]),
            ("with_menu", "Add interactive menu", &[
                ("items", "table"), ("origin_x", "number"), ("origin_y", "number"),
                ("font", "string"), ("font_size", "number"), ("item_spacing", "number"),
                ("use_screen_space", "boolean"),
            ]),
            ("with_menu_colors", "Set menu normal/selected colors (RGBA)", &[
                ("nr", "integer"), ("ng", "integer"), ("nb", "integer"), ("na", "integer"),
                ("sr", "integer"), ("sg", "integer"), ("sb", "integer"), ("sa", "integer"),
            ]),
            ...
            ("with_stuckto", "Attach entity to a target entity", &[
                ("target_entity_id", "integer"), ("follow_x", "boolean"), ("follow_y", "boolean"),
            ]),
            ...
            ("with_parent", "Set parent entity for transform hierarchy", &[("parent_id", "integer")]),
            ("with_camera_target", "Mark entity as camera follow target (higher priority wins)", &[
                ("priority", "integer?"),
            ]),
    ...
    pub(super) fn register_types_meta(&self) -> LuaResult<()> {
        ...
        let type_defs: &[LuaTypeDef] = &[
            ...
            ("Rect", "Axis-aligned rectangle", &[
                ("x", "number", false, None), ("y", "number", false, None),
                ("w", "number", false, None), ("h", "number", false, None),
            ]),
            ...
            ("DigitalInputs", "All digital button states", &[
                ...
                ("main_up", "DigitalButtonState", false, Some("Raw WASD up (W key)")),
                ("main_down", "DigitalButtonState", false, Some("Raw WASD down (S key)")),
                ("main_left", "DigitalButtonState", false, Some("Raw WASD left (A key)")),
                ("main_right", "DigitalButtonState", false, Some("Raw WASD right (D key)")),
                ("secondary_up", "DigitalButtonState", false, Some("Raw arrow up key")),
                ("secondary_down", "DigitalButtonState", false, Some("Raw arrow down key")),
                ("secondary_left", "DigitalButtonState", false, Some("Raw arrow left key")),
                ("secondary_right", "DigitalButtonState", false, Some("Raw arrow right key")),
                ("debug", "DigitalButtonState", false, Some("Debug toggle (F11)")),
                ("fullscreen", "DigitalButtonState", false, Some("Fullscreen toggle (F10)")),
            ]),
            ("AnalogInputs", "Analog input values (mouse, scroll)", &[
                ("scroll_y", "number", false, Some("Mouse wheel delta (positive=up, negative=down)")),
                ("mouse_x", "number", false, Some("Cursor X in game-space (0..render_width, letterbox-corrected)")),
                ("mouse_y", "number", false, Some("Cursor Y in game-space (0..render_height, letterbox-corrected)")),
                ("mouse_world_x", "number", false, Some("Cursor X in world-space (after camera transform, matches MapPosition)")),
                ("mouse_world_y", "number", false, Some("Cursor Y in world-space (after camera transform, matches MapPosition)")),
            ]),
            ...
            ("ParticleEmitterConfig", "Particle emitter configuration table", &[
                ...
                ("shape", "string|table", true, Some("Emitter shape: 'point' or table {type='rect', width, height}")),
                ...
                ("ttl", "number|table", true, Some("{min, max}, number, or 'none'")),
            ]),
    ...
    pub(super) fn register_enums_meta(&self) -> LuaResult<()> {
        ...
        let enum_defs: &[(&str, &str, &[&str])] = &[
            ...
            ("LoopMode", "Tween loop mode", &["once", "loop", "ping_pong"]),
            ("BoxSide", "Collision side", &["left", "right", "top", "bottom"]),
            ("ComparisonOp", "Comparison operator for animation rules", &["lt", "le", "gt", "ge", "eq", "ne"]),
            ...
            ("EmitterShape", "Particle emitter shape type", &["point", "rect"]),
            ("TtlSpec", "Time-to-live specification (number, {min,max} table, or 'none')", &["none"]),
            ("Category", "Function category", &[
                "base", "asset", "spawn", "audio", "signal", "phase", "entity",
                "group", "tilemap", "camera", "collision", "animation", "render",
            ]),
        ];
    ...
    pub(super) fn register_callbacks_meta(&self) -> LuaResult<()> {
        ...
        let callback_defs: &[CbDef] = &[
            CbDef { name: "on_setup", description: "Called once during game setup for asset loading",
                params: &[], returns: None, context: Some("setup"), note: None },
            CbDef { name: "on_enter_play", description: "Called when entering Playing state",
                params: &[], returns: None, context: Some("play"), note: None },
            CbDef { name: "on_switch_scene", description: "Called when switching scenes",
                params: &[("scene", "string")], returns: None, context: Some("play"), note: None },
            CbDef { name: "on_update_<scene>", description: "Called each frame during a scene",
            ...
            CbDef { name: "phase_on_enter", description: "Called when entering a phase",
                params: &[("ctx", "EntityContext"), ("input", "InputSnapshot")],
                returns: Some("string?"), context: Some("play"),
                note: Some("Return phase name to trigger transition") },
            CbDef { name: "phase_on_update", description: "Called each frame during a phase",
                params: &[("ctx", "EntityContext"), ("input", "InputSnapshot"), ("dt", "number")],
                returns: Some("string?"), context: Some("play"),
                note: Some("Return phase name to trigger transition") },
            CbDef { name: "phase_on_exit", description: "Called when exiting a phase",
                params: &[("ctx", "EntityContext")],
                returns: None, context: Some("play"), note: None },
            CbDef { name: "timer_callback", description: "Called when a Lua timer fires",
                params: &[("ctx", "EntityContext"), ("input", "InputSnapshot")],
                returns: None, context: Some("play"), note: None },
            CbDef { name: "collision_callback", description: "Called when two colliding groups overlap",
                params: &[("ctx", "CollisionContext")],
                returns: None, context: Some("play"), note: None },
            CbDef { name: "menu_callback", description: "Called when a menu item is selected",
