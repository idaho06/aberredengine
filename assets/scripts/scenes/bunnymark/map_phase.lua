-- scenes/bunnymark/map_phase.lua
-- Bunnymark variant: MapPosition + per-entity Phase callback.
...
local function bunny_map_phase_on_update(ctx, _, dt)
    ...
    engine.entity_set_position(ctx.id, s.x, s.y)
end
...
--- @param dt number
local function on_update_bunnymark_map_phase(input, dt)
    ...
    if input.digital.action_1.pressed then
        ...
        for _ = 1, C.BUNNIES_PER_FRAME do
            ...
            local key          = "mp" .. bunny_key_counter
            ...
            engine.spawn()
                ...
                :with_phase({
                    initial = "move",
                    phases  = { move = { on_update = "bunny_map_phase_on_update" } },
                })
                :register_as(key)
                :build()
...
M._callbacks = {
    on_update_bunnymark_map_phase  = on_update_bunnymark_map_phase,
    bunny_map_phase_on_update      = bunny_map_phase_on_update,
}
