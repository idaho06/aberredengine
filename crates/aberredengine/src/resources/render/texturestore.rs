...
/// (via `RenderAssetCmd::RemoveTexture`, since logic-side code has no access
/// to this store at all) — the removed `Texture2D` value's `Drop` impl
/// releases the underlying GPU resource as a side effect of being dropped,
/// not of `remove`'s own code.
///
/// Remains a plain `Resource` (not `NonSend`) — the render world owns it
/// exclusively, so `NonSend` would add nothing.
