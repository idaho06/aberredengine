...
use aberredengine::resources::input_bindings::{InputBinding, InputBindings, key_from_str, key_to_str};
...
/// All 14 InputAction discriminants
...
/// The default binding count should be exactly 14 (one per action).
#[test]
fn test_default_bindings_count() {
    let bindings = InputBindings::default();
    assert_eq!(bindings.map.len(), 14, "Expected 14 default action bindings");
}
